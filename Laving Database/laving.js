...
client.on('ready', async () => {
  client.user.setStatus(`${ayarlar.setStatus}`);
  setInterval(() => {
      const laving3 = [`${ayarlar.botDurum}`];
  const laving2 = Math.floor(Math.random() * (laving3.length));
  client.user.setActivity(`${laving3[laving2]}`, {type: "LISTENING"});
}, 10000);
let laving = client.channels.cache.get(ayarlar.voiceChannelID);
let channel = ayarlar.urlLOG;
if (laving) laving.join().catch(err => console.error("Ses kanalına giriş başarısız"));
console.log(`${client.user.tag} Kullanıma Hazır.`);
setInterval(function(){
  let laving4 = new MessageEmbed().setColor("19dac9").setAuthor("URL İnfo")
  client.guilds.cache.get(ayarlar.guildID).channels.cache.get(channel).send(laving4.setDescription(`Vanity URL : \`\`${ayarlar.sunucuURL}\`\` olarak güncellendi.`))
  },900000);
  setInterval(function(){
      URL()
  },1350)
